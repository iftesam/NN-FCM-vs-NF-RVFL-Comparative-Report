...
class _TorchFCM(nn.Module):
    ...
    def __init__(self, n_features, n_rules, mlp_units, dropout):
        ...
        self.fc5 = nn.Linear(mlp_units, mlp_units); self.bn5 = nn.BatchNorm1d(mlp_units)
        self.fc6 = nn.Linear(mlp_units, mlp_units); self.bn6 = nn.BatchNorm1d(mlp_units)
    ...
    def forward(self, X):
        ...
        h = self.dropout(self.relu(self.bn5(self.fc5(h))) + h)
        h = self.dropout(self.relu(self.bn6(self.fc6(h))) + h)

        out_heads = torch.stack([head(h) for head in self.heads], dim=2)
        return out_heads.mean(dim=2).squeeze(1)
                                         # [B]
